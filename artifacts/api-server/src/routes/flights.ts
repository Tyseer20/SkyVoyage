...
function minutesBetween(start: string, end: string) {
  return Math.max(
    1,
    Math.round(
      (new Date(end).valueOf() - new Date(start).valueOf()) / 60_000,
    ),
  );
}
...
async function duffelSearch(
  criteria: ReturnType<typeof SearchFlightsQueryParams.parse>,
) {
      ...
      body: JSON.stringify({
        data: {
          ...
          passengers: Array.from(
            { length: criteria.passengers },
            () => ({ type: "adult" }),
          ),
          cabin_class:
            criteria.cabin === "premium-economy"
              ? "premium_economy"
              : criteria.cabin,
          max_connections:
            criteria.stops === "direct"
              ? 0
              : criteria.stops === "one-stop"
                ? 1
                : undefined,
        },
      }),
  ...
  if (!response.ok) {
    throw new Error(
      `Duffel offer request failed with ${response.status}`,
    );
  }
  ...
  if (!request.data?.id) {
    throw new Error("Duffel did not return an offer request");
  }

  const offersResponse = await fetch(
    `https://api.duffel.com/air/offers?offer_request_id=${request.data.id}&limit=30&sort=total_amount`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
        "Duffel-Version": "v2",
  ...
  if (!offersResponse.ok) {
    throw new Error(
      `Duffel offers request failed with ${offersResponse.status}`,
    );
  }
  ...
  try {
    ...
    const flights = offers
      .map((offer, index) => {
        ...
        // Get the last segment without using Array.prototype.at()
        // for compatibility with the deployment TypeScript environment.
        const last = segments[segments.length - 1] ?? first;

        const carrier = first?.marketing_carrier;

        const duration = minutesBetween(
          first?.departing_at ?? criteria.departureDate,
          last?.arriving_at ?? criteria.departureDate,
        );
        ...
        const price = Math.round(
          Number(offer.total_amount) /
            Math.max(1, criteria.passengers),
        );
        ...
        return {
          ...
          flightNumber: `${carrier?.iata_code ?? "SV"} ${
            first?.marketing_carrier_flight_number ?? index + 1
          }`,
          originCity: first?.origin?.name ?? criteria.origin,
          originCode: first?.origin?.iata_code ?? criteria.origin,
          destinationCity:
            last?.destination?.name ?? criteria.destination,
          destinationCode:
            last?.destination?.iata_code ?? criteria.destination,
          departureTime: first
